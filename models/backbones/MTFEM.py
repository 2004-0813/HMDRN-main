...
class MaskedAttention(nn.Module):
    ...
    def forward(self, x, is_query=False):
        ...
        if is_query:
            q_scores = torch.sum(q * q, dim=-1, keepdim=True)
            q_mean = torch.mean(q_scores, dim=-2, keepdim=True)
            q_mask = (q_scores > q_mean).float()
            q = q * q_mask
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src: torch.Tensor, is_query=False) -> torch.Tensor:
        src = src + self.self_attn(self.pre_norm(src), is_query=is_query)
        src = self.norm1(src)
...
class MTFEM(nn.Module):
    def __init__(self,
                 sequence_length=25,
                 embedding_dim=64,
                 *args, **kwargs):
        super(MTFEM, self).__init__()
    ...
    def forward(self, x, is_query=False):
        ...
        x = self.transformer(x, is_query=is_query)
        return x